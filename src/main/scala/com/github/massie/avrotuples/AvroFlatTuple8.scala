...
object AvroFlatTuple8 {

  val SCHEMA$ = AvroTupleSchemas.flatSchemas(7)

  val reader = new SpecificDatumReader[AvroFlatTuple8[_, _, _, _, _, _, _, _]](SCHEMA$)
  val writer = new SpecificDatumWriter[AvroFlatTuple8[_, _, _, _, _, _, _, _]](SCHEMA$)

  def readFromInputStream(tuple: AvroFlatTuple8[_, _, _, _, _, _, _, _], in: InputStream) = {
    AvroFlatTuple8.reader.read(tuple, DecoderFactory.get.directBinaryDecoder(in, null))
  }

  def writeToOutputStream(tuple: AvroFlatTuple8[_, _, _, _, _, _, _, _], out: OutputStream) = {
    AvroFlatTuple8.writer.write(tuple, EncoderFactory.get.directBinaryEncoder(out, null))
  }

  def fromInputStream(in: InputStream) : AvroFlatTuple8[_, _, _, _, _, _, _, _] = {
    readFromInputStream(null.asInstanceOf[AvroFlatTuple8[_, _, _, _, _, _, _, _]], in)
  }

  def fromBytes(bytes: Array[Byte]): AvroFlatTuple8[_, _, _, _, _, _, _, _] = {
...
final case class AvroFlatTuple8[T1, T2, T3, T4, T5, T6, T7, T8](
    @transient var _1: T1,
    @transient var _2: T2,
    @transient var _3: T3,
    @transient var _4: T4,
    ...
    @transient var _7: T7,
    @transient var _8: T8)
  extends Product8[T1, T2, T3, T4, T5, T6, T7, T8] with SpecificRecord with KryoSerializable with Externalizable {
  ...
  def update(n1: T1, n2: T2, n3: T3, n4: T4, n5: T5, n6: T6, n7: T7, n8: T8): AvroFlatTuple8[T1, T2, T3, T4, T5, T6, T7, T8] = {
    _1 = n1
    _2 = n2
    _3 = n3
    _4 = n4
  ...
  override def getSchema: Schema = AvroFlatTuple8.SCHEMA$

  override def toString: String = "(" + _1 + "," + _2 + "," + _3 + "," + _4 + "," + _5 + "," + _6 + "," + _7 + "," + _8 + ")"

  def toBytes: Array[Byte] = {
    val byteStream = new ByteArrayOutputStream()
    AvroFlatTuple8.writeToOutputStream(this, byteStream)
    byteStream.flush()
  ...
  override def readExternal(in: ObjectInput): Unit = {
    AvroFlatTuple8.readFromInputStream(this, ExternalizableInput(in))
  }

  override def writeExternal(out: ObjectOutput): Unit = {
    AvroFlatTuple8.writeToOutputStream(this, ExternalizableOutput(out))
  }

  override def write(kryo: Kryo, output: Output): Unit = {
    AvroFlatTuple8.writeToOutputStream(this, output.getOutputStream)
  }

  override def read(kryo: Kryo, input: Input): Unit = {
    AvroFlatTuple8.readFromInputStream(this, input.getInputStream)
  }
