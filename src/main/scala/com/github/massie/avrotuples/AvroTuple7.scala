...
object AvroTuple7 {
  ...
  val SCHEMA$ = AvroTupleSchemas.SCHEMAS(6)
  val FLAT_SCHEMA = AvroTupleSchemas.FLAT_SCHEMAS(6)

  val reader = new SpecificDatumReader[AvroTuple7[_, _, _, _, _, _, _]](SCHEMA$)
  val writer = new SpecificDatumWriter[AvroTuple7[_, _, _, _, _, _, _]](SCHEMA$)

  def readFromInputStream(tuple: AvroTuple7[_, _, _, _, _, _, _], in: InputStream) = {
    AvroTuple7.reader.read(tuple, DecoderFactory.get.directBinaryDecoder(in, null))
  }

  def writeToOutputStream(tuple: AvroTuple7[_, _, _, _, _, _, _], out: OutputStream) = {
    AvroTuple7.writer.write(tuple, EncoderFactory.get.directBinaryEncoder(out, null))
  }

  def fromInputStream(in: InputStream) : AvroTuple7[_, _, _, _, _, _, _] = {
    readFromInputStream(null.asInstanceOf[AvroTuple7[_, _, _, _, _, _, _]], in)
  }

  def fromBytes(bytes: Array[Byte]): AvroTuple7[_, _, _, _, _, _, _] = {
...
case class AvroTuple7[T1, T2, T3, T4, T5, T6, T7](
  ...
  def update(n1: T1, n2: T2, n3: T3, n4: T4, n5: T5, n6: T6, n7: T7): AvroTuple7[T1, T2, T3, T4, T5, T6, T7] = {
    _1 = n1
    _2 = n2
    _3 = n3
    _4 = n4
  ...
  override def getSchema: Schema = AvroTuple7.SCHEMA$

  override def toString: String = "(" + _1 + "," + _2 + "," + _3 + "," + _4 + "," + _5 + "," + _6 + "," + _7 + ")"

  def toBytes: Array[Byte] = {
    val byteStream = new ByteArrayOutputStream()
    AvroTuple7.writeToOutputStream(this, byteStream)
    byteStream.flush()
  ...
  override def readExternal(in: ObjectInput): Unit = {
    AvroTuple7.readFromInputStream(this, ExternalizableInput(in))
  }
