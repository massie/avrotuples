...
object AvroTuple10 {
  ...
  val SCHEMA$ = AvroTupleSchemas.SCHEMAS(9)
  val FLAT_SCHEMA = AvroTupleSchemas.FLAT_SCHEMAS(9)

  val reader = new SpecificDatumReader[AvroTuple10[_, _, _, _, _, _, _, _, _, _]](SCHEMA$)
  val writer = new SpecificDatumWriter[AvroTuple10[_, _, _, _, _, _, _, _, _, _]](SCHEMA$)

  def readFromInputStream(tuple: AvroTuple10[_, _, _, _, _, _, _, _, _, _], in: InputStream) = {
    AvroTuple10.reader.read(tuple, DecoderFactory.get.directBinaryDecoder(in, null))
  }

  def writeToOutputStream(tuple: AvroTuple10[_, _, _, _, _, _, _, _, _, _], out: OutputStream) = {
    AvroTuple10.writer.write(tuple, EncoderFactory.get.directBinaryEncoder(out, null))
  }

  def fromInputStream(in: InputStream) : AvroTuple10[_, _, _, _, _, _, _, _, _, _] = {
    readFromInputStream(null.asInstanceOf[AvroTuple10[_, _, _, _, _, _, _, _, _, _]], in)
  }

  def fromBytes(bytes: Array[Byte]): AvroTuple10[_, _, _, _, _, _, _, _, _, _] = {
...
final case class AvroTuple10[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10](
    @transient var _1: T1,
    @transient var _2: T2,
    @transient var _3: T3,
    @transient var _4: T4,
  ...
  def update(n1: T1, n2: T2, n3: T3, n4: T4, n5: T5, n6: T6, n7: T7, n8: T8, n9: T9, n10: T10): AvroTuple10[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10] = {
    _1 = n1
    _2 = n2
    _3 = n3
    _4 = n4
  ...
  override def getSchema: Schema = AvroTuple10.SCHEMA$

  override def toString: String = "(" + _1 + "," + _2 + "," + _3 + "," + _4 + "," + _5 + "," + _6 + "," + _7 + "," + _8 + "," + _9 + "," + _10 + ")"

  def toBytes: Array[Byte] = {
    val byteStream = new ByteArrayOutputStream()
    AvroTuple10.writeToOutputStream(this, byteStream)
    byteStream.flush()
  ...
  override def readExternal(in: ObjectInput): Unit = {
    AvroTuple10.readFromInputStream(this, ExternalizableInput(in))
  }

  override def writeExternal(out: ObjectOutput): Unit = {
    AvroTuple10.writeToOutputStream(this, ExternalizableOutput(out))
  }

  override def write(kryo: Kryo, output: Output): Unit = {
    AvroTuple10.writeToOutputStream(this, output.getOutputStream)
  }

  override def read(kryo: Kryo, input: Input): Unit = {
    AvroTuple10.readFromInputStream(this, input.getInputStream)
  }
